...
const imageHash = config => {
  ...
  sortConfig(config).signers.forEach(a => imageHash = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['bytes32', 'uint8', 'address'], [imageHash, a.weight, a.address])));
  return imageHash;
...
class SequenceUtilsFinder {
  constructor(authProvider) {
    ...
    this.findConfigForImageHash = async function (context, image, knownConfigs = []) {
      logger.info(`[findConfigForImageHash] image:${image}`); // Lookup config in known configurations
