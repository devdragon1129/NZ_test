...
const imageHash = config => {
  let imageHash = ethers.ethers.utils.solidityPack(['uint256'], [config.threshold]);
  sortConfig(config).signers.forEach(a => imageHash = ethers.ethers.utils.keccak256(ethers.ethers.utils.defaultAbiCoder.encode(['bytes32', 'uint8', 'address'], [imageHash, a.weight, a.address])));
  return imageHash;
  ...
  constructor(authProvider) {
    ...
    this.findCurrentConfig = async function (args) {
      ...
      const {
        imageHash,
        config
      } = await _this.findCurrentImageHash(context, provider, address, knownConfigs);
      if (imageHash === undefined) return {
        config: undefined
      }; // Get config for that imageHash

      const found = await _this.findConfigForImageHash(context, imageHash, config ? [config, ...knownConfigs] : knownConfigs);
    ...
    this.findConfigForImageHash = async function (context, image, knownConfigs = []) {
      utils.logger.info(`[findConfigForImageHash] image:${image}`); // Lookup config in known configurations

      const found = knownConfigs.find(kc => imageHash(kc) === image);
      if (found) return found; // Load index for last imageHash update
    ...
    this.findCurrentImageHash = async function (context, provider, address, knownConfigs = []) {
      ...
      if (currentImageHash[0] !== undefined) return {
